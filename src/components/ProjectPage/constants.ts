...
export const automatedVoiceControlledChessboard = {
    ...
    content: {
        english: 'A couple of friends and I joined forces to build Wizard\'s Chess, but without the magic. A set of ' +
            'stepper motors were combined to allow for the two-dimensional linear motion of an electromagnet, which ' +
            'activated and deactivated to move the chess pieces. A speech-to-text library was linked to the ' +
            'mechanical components using an Arduino. I implemented a stateful move-checking algorithm that would ' +
            'determine if the players\' voice commands represented legal moves in classical chess.',
        french: 'Quelques amis et moi nous sommes réunis pour construire le Wizard’s Chess, mais sans la magique. ' +
            'Des moteurs pas-à-pas ont été assemblés pour permettre le mouvement linéaire bi-dimensionnel d’un ' +
            'électroaimant, qui a été allumé et éteint pour déplacer les pièces d’échecs. Une librairie ' +
...
export const yrdsbStudentPlannerApp = {
    ...
    content: {
        english: 'I designed and built an Android mobile app that provided high school students in York Region, ' +
            'Ontario the ability to get school announcements and event information delivered straight to their ' +
            'phones. The app also had an agenda feature where students could track their assignment deadlines! ' +
            'An SMTP email verification system was established to authenticate students without the need for a ' +
            'password. Unfortunately, when COVID hit, plans to publish the app with the school board were ' +
            'cancelled.',
...
export const idealGasLawsSimulation = {
    ...
    content: {
        english: 'A simulation of ideal gas particles moving in 3D space was constructed, and its quantifiable ' +
            'observations were tested against classical kinematic equations to verify their relations with the ' +
            'ideal gas laws. Various experimental parameters, such as temperature and container volume, could be ' +
...
export const minecraftPlugins = {
    title: {
        english: 'Minecraft Plugins',
        french: 'Plugins pour Minecraft',
    },
    content: {
